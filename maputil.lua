...
function ShowClosestNodeToPlayer()
	...
	local x = node.x
	local y = node.y
	GetWorld().minimap.MiniMap:ShowArea(x,0,y,15)
end
...
function PlayerSub(count)
	...
	for i=1,#ressorted do
		...
		for pt = 1,len/5 do
			...
			GetWorld().minimap.MiniMap:ShowArea(x,0,y,8)
		end
...
function MapHideAll()
	GetWorld().minimap.MiniMap:ClearRevealedAreas()
end

function DrawWalkableGrid(graph)
	local graph = graph or GetWorld().topology
	...
	for i=#graph.flattenedEdges,1,-1 do
		if graph.flattenedEdges[i] then
			...
			for pt = 1,len/5 do
				local x = srcx + dx * 5 * pt
				local y = srcy + dy * 5 * pt
				GetWorld().minimap.MiniMap:ShowArea(x,0,y,3)
			end
		end
	end
end
...
function ReconstructTopology(graph)
	...
	-- give the nodes a key number for reference
	local num = 1
	for _,node in pairs(graph.nodes) do
		node.idx = num
		num = num + 1
	...
	for i=#flattenedEdges,1,-1 do
		...
		elseif #nodeIndices == 2 then
			...
			local endpos = Point(node2.x,0,node2.y)
			-- get center x and y
			local startposcent = Point(node1.cent[1],0,node1.cent[2])			
			local endposcent = Point(node2.cent[1],0,node2.cent[2])			
			
			-- pathfind multiple ways for a better chance at accuracy
			if not GetWorld().Pathfinder:IsClear(startpos.x, startpos.y, startpos.z,
		                                                 endpos.x, endpos.y, endpos.z,
		                                                 {ignorewalls = true, ignorecreep = true}) and 
		       not GetWorld().Pathfinder:IsClear(startposcent.x, startposcent.y, startposcent.z,
		                                                 endposcent.x, endposcent.y, endposcent.z,
