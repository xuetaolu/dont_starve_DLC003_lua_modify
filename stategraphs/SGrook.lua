...
--SpawnWaves(inst, numWaves, totalAngle, waveSpeed, wavePrefab, initialOffset, idleTime, instantActive, random_angle)
    ...
    State{  name = "run",
            ...
            timeline=
            {
                ...
                TimeEvent(5*FRAMES, function(inst)
                                        SpawnPrefab("ground_chunks_breaking").Transform:SetPosition(inst.Transform:GetWorldPosition())
                                        if inst:HasTag("wavemaker") and inst.onwater then
                                            SpawnWaves(inst, 6, 360, 3, "wave_ripple") -- initialOffset, idleTime, instantActive, random_angle)
                                        end
                                    end ),
            },
