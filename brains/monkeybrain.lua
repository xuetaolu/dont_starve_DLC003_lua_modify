...
local function AnnoyLeader(inst)
    ...
    for k,v in pairs(ents) do
        if v.components.inventoryitem and v.components.inventoryitem.canbepickedup and not v.components.inventoryitem:IsHeld() and not v:HasTag("irreplaceable") and v:GetTimeAlive() < 5 then
    ...
    if inst.canlootchests then
        ...
        for k,v in pairs(ents) do
            ...
            if cont and not v:HasTag("aquatic") then
        ...
        if #items > 0 then
            ...
            act.validfn = function() return (item.components.inventoryitem and item.components.inventoryitem:IsHeld() and not item:HasTag("irreplaceable")) end
            return act
        end
    end
end
...
local function HarassPlayer(inst)
    ...
    return inst.harassplayer and not (target and target.components.driver and target.components.driver:GetIsDriving())
end
...
function MonkeyBrain:OnStart()
    ...
    local root = PriorityNode(
    {
        ...
        --Following
        WhileNode(function() return HarassPlayer(self.inst) end, "Annoy Leader", 
        ...
        --Doing nothing
        WhileNode(function() return HarassPlayer(self.inst)  end, "Wander Around Leader", 
