...
--  return string.format("state: %s, progress: %2.2f/%2.2f, discomfort: %2.2f/%2.2f", self.state, self.progress, self.state == "unhatched" and self.cracktime or self.hatchtime, self.discomfort, self.hatchfailtime)
   return string.format("state: %s, progress: %2.2f, discomfort: %2.2f", self.state, self.progress, self.discomfort)    
end
...
function Hatchable:OnUpdate(dt)
    ...
    if self.state == "unhatched" then
		if hasfire then
			...
			if self.cracktime and self.progress >= self.cracktime then
				self.progress = 0
                self:OnState("crack")
			end
		else
    ...
    if self.state == "comfy" then
        ...
        if self.hatchfailtime and self.discomfort >= self.hatchfailtime then
            self:StopUpdating()
            self:OnState("dead")
        end
--        if self.inst:HasTag("uncomfy_backup_progress") then
