...
function BaseHassler:PhysicallySpawnHassler(name)
	local h = self.hasslers[name]
...
function BaseHassler:ScheduleSpawnHassler(name)
	self.scheduledSpawnHasslers[name] = (self.scheduledSpawnHasslers[name] or 0) + 1
	return true
end
...
function BaseHassler:SpawnHassler(name)
	...
	if GetInteriorSpawner():IsPlayerConsideredInside() then
		return self:ScheduleSpawnHassler(name)
	else
		return self:PhysicallySpawnHassler(name)
	end
end

function BaseHassler:HandleScheduledSpawns()
	if not GetInteriorSpawner():IsPlayerConsideredInside() then
		for name, count in pairs(self.scheduledSpawnHasslers) do
			local spawnsLeft = 0
			for i=1,count do
				if not self:PhysicallySpawnHassler(name) then
					spawnsLeft = spawnsLeft + 1
				end
			end
			self.scheduledSpawnHasslers[name] = spawnsLeft
		end
	end
end
...
function BaseHassler:OnSave()
	...
	data.scheduledSpawnHasslers = self.scheduledSpawnHasslers
	return data
end
...
function BaseHassler:OnLoad(data)
	if data and data.hasslers then
		...
		if data.scheduledSpawnHasslers then
			self.scheduledSpawnHasslers = data.scheduledSpawnHasslers
		end
	end
end
