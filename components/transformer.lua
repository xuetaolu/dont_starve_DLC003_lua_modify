...
function Transformer:Transform()
	self.inst:DoTaskInTime(math.random(), function() 
		...
		if self.inst.components.citypossession and self.inst.components.citypossession.cityID then
			transformedPrefab:AddComponent("citypossession")
			transformedPrefab.components.citypossession:SetCity(self.inst.components.citypossession.cityID)
		end
		self.inst:Remove()
	end)
end
...
function Transformer:Revert()
	self.inst:DoTaskInTime(math.random(), function() 
		...
		if obj then
			...
			if self.inst.components.citypossession and self.inst.components.citypossession.cityID then
				obj:AddComponent("citypossession")
				obj.components.citypossession:SetCity(self.inst.components.citypossession.cityID)
			end
			self.inst:Remove()
		end
	end)
