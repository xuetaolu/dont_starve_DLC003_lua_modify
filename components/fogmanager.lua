...
local FogManager = Class(function(self, inst)
	self.inst = inst
	self.entities =	{}
...
function FogManager:GetDebugString()
...
function FogManager:StartMakeWorldWet(time)
...
function FogManager:MakeWorldDry()
	self.entities = {}
...
function FogManager:MakeEntityDry(inst)
	--print(inst, " is sheltered! Making dry.")
	self.entities[inst] = inst
end

function FogManager:MakeEntityWet(inst)
...
function FogManager:IsEntityWet(inst)
...
function FogManager:IsEntityDry(inst)
...
function FogManager:IsWorldWet()
	return self.world_wet
end

function FogManager:OnSave()
	local data = {}
...
function FogManager:OnLoad(data)
	if data then
...
function FogManager:CheckForChange(data)
...
function FogManager:GetWorldMoisture()
	return self.moisture
end

function FogManager:DoDelta(num)
...
function FogManager:SetWorldMoistureLevel(num)
	self.moisture = math.clamp(num, self.moistureclamp.min, self.moistureclamp.max)
end

function FogManager:GetWorldMoistureRate()
...
function FogManager:GetDryingRate()
...
function FogManager:OnUpdate(dt)
...
function FogManager:LongUpdate(dt)
	self:OnUpdate(dt)
end
