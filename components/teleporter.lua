...
function Teleporter:Activate(doer)
    ...
    local tile = GetVisualTileType(doer.Transform:GetWorldPosition())
	...
	if doer.components.leader then
		for follower,v in pairs(doer.components.leader.followers) do
			print("player",is_water)
			print("follower",follower.prefab)

			if (not is_water and not follower:HasTag("seacreature") ) or 
			   (    is_water and (follower:HasTag("seacreature") or follower:HasTag("flying") or follower:HasTag("amphibious")) ) then
			   	print(is_water,follower:HasTag("seacreature") , follower:HasTag("flying") , follower:HasTag("amphibious"))
