...
function Guardian:SummonGuardian(override, position_override)
	...
	local guard = FindEntity(self.inst, 40, function(ent) return ent.prefab == self.prefab end)
	...
	if not self.guardian then
		local pt = position_override or self.inst:GetPosition()
