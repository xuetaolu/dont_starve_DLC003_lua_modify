local FlowerSpawner_rainforest = Class(function(self, inst)
    self.inst = inst
...
function FlowerSpawner_rainforest:Enable(enable)
	if enable then
...
function FlowerSpawner_rainforest:GetSpawnTime()
	return self.timetospawn + (math.random() * self.timetospawn_variation)
end

function FlowerSpawner_rainforest:CheckTileCompatibility(tile)
...
function FlowerSpawner_rainforest:GetSpawnPoint(player)
    ...
    for i = 1, steps do
		...
		#TheSim:FindEntities(try_pos.x, try_pos.y, try_pos.z, 1, nil, {"falling", "FX", "NOCLICK", "DECOR", "INLIMBO"}) <= 0 then
...
function FlowerSpawner_rainforest:SpawnFlower(pt)
	local flower = SpawnPrefab(self.prefab)
	flower.Transform:SetPosition(pt:Get())
end

function FlowerSpawner_rainforest:OnUpdate( dt )
    if self.active then
...
function FlowerSpawner_rainforest:GetDebugString()
	return "Next spawn: "..tostring(self.spawntimer)
end

function FlowerSpawner_rainforest:OnSave()
    local data = {}
...
function FlowerSpawner_rainforest:OnLoad(data)
    if data then
...
function FlowerSpawner_rainforest:SpawnModeNever()
...
function FlowerSpawner_rainforest:SpawnModeHeavy()
    self.timetospawn_variation = 5
    self.timetospawn = 10
end

function FlowerSpawner_rainforest:SpawnModeMed()
    self.timetospawn_variation = 10
    self.timetospawn = 20
end

function FlowerSpawner_rainforest:SpawnModeLight()
    self.timetospawn_variation = 15
    self.timetospawn = 60
end


return FlowerSpawner_rainforest
