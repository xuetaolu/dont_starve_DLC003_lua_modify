...
function MaxwellTalker:Initialize(nohide)
	...
	if self.inst.speech and self.inst.speech.disableplayer then
        ...
        local dist = 4
        if self.setdist then
        	dist = self.setdist 
        end
        local pt = Vector3(self.inst.wilson.Transform:GetWorldPosition()) + TheCamera:GetRightVec()*dist
	...
	if self.inst.speech then
		...
		for k, section in ipairs(self.inst.speech) do --the loop that goes on while the speech is happening
	        ...
	        if section.string then --If there is speech to be said, it displays the text and overwrites any custom anims with the talking anims
	        	...
	        	if self.inst.speech and self.inst.speech.dialoganim or section.dialoganim then 
	        		local speechanim = self.inst.speech.dialoganim
	        		if section.dialoganim then
	        			speechanim = section.dialoganim
	        		end
	        		if self.inst.speech.dialogpreanim then
	        			self.inst.AnimState:PushAnimation(self.inst.speech.dialoganim, true)
	        		else
	        			if section.postanim then
	        				self.inst.AnimState:PlayAnimation(speechanim)
	        				self.inst.AnimState:PushAnimation(section.postanim, true)
	        			else
	        				self.inst.AnimState:PlayAnimation(speechanim, true)
	        			end
	        		end
        		end
