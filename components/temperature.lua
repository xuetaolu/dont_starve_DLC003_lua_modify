...
function Temperature:DeltaTemp(delta)
	local old = self.current
	self.current =  self.current + delta
	self.inst:PushEvent("temperaturedelta", {last = old, new = self.current})		
end
...
function Temperature:GetDebugString()
    return string.format("%2.2fC at %2.2f (delta: %2.2f) overheat:%2.2f", self:GetCurrent(), self.rate, self.last_real_delta, self.overheattemp)
end
