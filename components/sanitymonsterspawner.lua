...
function SanityMonsterSpawner:AddMonsterOfType(monstername, spawnPos)
	...
	monster:ListenForEvent( "mountboat", function() 
		if not monster:HasTag("aquatic") then
			monster.sg:GoToState("disappear")
		end
	end, self.inst )
	monster:ListenForEvent( "dismountboat", function() 
		if monster:HasTag("aquatic") then
			monster.sg:GoToState("disappear")
		end
	end, self.inst )
...
function SanityMonsterSpawner:FindSpawnPoint()
	...
	if self.inst:HasTag("aquatic") then
		offset = FindWaterOffset(pos, theta, radius, 12) or offset
	else
		offset = FindGroundOffset(pos, theta, radius, 12) or offset
	end
...
function SanityMonsterSpawner:UpdateMonsters(dt)
	...
	if self.currenttargetpop > self.currentpop and (not self.spawntimer or self.spawntimer <= 0) then
		...
		local tile = pt and GetMap():GetTileAtPoint(pt:Get())

		local prefab = self:GetCreatureForTile(tile)
...
function SanityMonsterSpawner:GetCreatureForTile(tile)
	local sanity = self.inst.components.sanity:GetPercent()
	local prefab = "crawlinghorror"

	if GetWorld().Map:IsWater(tile) then
		prefab = "swimminghorror"
	else
