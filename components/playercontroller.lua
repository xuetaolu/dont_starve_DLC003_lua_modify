...
--print("-------------",GetPlayer():GetPosition())
	...
	if not self.inst.sg:HasStateTag("busy") then
    	...
    	---- playercontroller.lua :OnUpdate(dt)
    	-- if not self.inst.sg:HasStateTag("busy") then ...
    	-- end
    	-- 插入 else 部分，执行 self:DoDirectWalking2(dt)
    	self:DoDirectWalking2(dt)
    end
...
function PlayerController:DoDirectWalking2(dt)
	...
	if dir then
		...
		local direction = ang
        locomotor.inst.Transform:SetRotation(direction)
        locomotor.inst.Physics:SetMotorVel(locomotor:GetRunSpeed(),0,0)
        -- locomotor.wantstomoveforward = true
        locomotor.wantstorun = true
