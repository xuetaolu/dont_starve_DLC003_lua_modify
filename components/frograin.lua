...
function FrogRain:OnLoad(data)
	if data ~= nil then
		...
		self.queuedfrogs = data.queuedfrogs or self.queuedfrogs
	end
end
...
function FrogRain:OnUpdate( dt )
    ...
    if maincharacter then
		...
		if self.timetospawn <= 0 and self.frog_count < self.frogcap then
			...
			if SaveGameIndex:GetCurrentMode() == "adventure" and 
				...
				self.queuedfrogs = self.queuedfrogs +1
			end
	...
	if 	self.queuedfrogs > 0 and not TheCamera.interior then				
		for i=1,self.queuedfrogs do
