...
function Key:CollectUseActions(doer, target, actions)
	if target.components.lock and not target.components.lock:IsStuck() and target.components.lock:CompatableKey(self.keytype) and target.components.lock:IsLocked() then
