...
function BaseHassler:OnSave()
	if not self.noserial then
		return 
		{
			...
			scheduledSpawnHasslers = self.scheduledSpawnHasslers
		}
	end
	self.noserial = false
end
...
function BaseHassler:OnLoad(data)
	...
	self.scheduledSpawnHasslers = data.scheduledSpawnHasslers or 0
end
...
function BaseHassler:ScheduleSpawnHassler()
	self.scheduledSpawnHasslers = (self.scheduledSpawnHasslers or 0) + 1
end
...
function BaseHassler:HandleScheduledSpawns()
	-- see if we can spawn what we had scheduled to spawn. If not, try again next frame
	if not GetInteriorSpawner():IsPlayerConsideredInside() then
		for i=1,self.scheduledSpawnHasslers do
			self:ReleaseHassler()
			self.scheduledSpawnHasslers = 0
		end
	else
		if self.scheduledSpawnHasslers > 0 then
			self.inst:DoTaskInTime(0, function()
											if self.inst.components.basehassler then
												self.inst.components.basehassler:HandleScheduledSpawns()
											end
									  end)
		end
	end
...
function BaseHassler:CeaseAttacks()
	...
	-- Just calling this so we get rescheduled for next frame if needed
	self:HandleScheduledSpawns()
end
