local LurePlantSpawner = Class(function(self, inst)
    ...
    self:Enable(false)
    if self.inst.components.seasonmanager:IsSpring() or
        self.inst.components.seasonmanager:IsGreenSeason() or
        self.inst.components.seasonmanager:IsDrySeason() then
    	self:Enable(true)
    end
    self.inst:ListenForEvent("seasonChange", function(inst, data) 
    	if data.season == "spring" or data.season == "green" or data.season == "dry" then
    		self:Enable(true)
    	else
    		self:Enable(false)
    	end    		
