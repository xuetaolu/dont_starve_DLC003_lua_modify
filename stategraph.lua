...
function StateGraphInstance:GoToState(statename, params)
    ...
    -- 魔改 onenter 不是玩家的情况下， attack 状态不能调用 Physics:Stop()
    if self.currentstate.onenter ~= nil then
        local need_nostop = self.inst ~= GetPlayer() and self.currentstate.name == "attack"
        if not need_nostop then
            self.currentstate.onenter(self.inst, params)
        else
            local old_Physics = self.inst.Physics

            local Physics = {}
            self.inst.Physics = Physics
            local funcList = { "GetRadius", "SetCollisionCallback", "Teleport", "SetVel", "SetMotorVel", "GetVelocity", "Stop", "SetDamping", "SetActive", "SetMotorVelOverride", "SetMass", "SetCapsule", "SetFriction", "SetRestitution", "ClearCollisionMask", "SetCollides", "CollidesWith", "SetCollisionGroup", "ClearMotorVelOverride", "GetMotorSpeed", "GetMotorVel", "SetSphere", "SetDontRemoveOnSleep", "SetRectangle", "SetCylinder", "IsActive", "GetCollisionGroup", "GetMass" }
            ...
            setmetatable( Physics, {
                ...
                __index=function(t,k)
                    ...
                    -- return old_Physics.k
                    local fn = function(arg1, ...)
                        return old_Physics[k](old_Physics, ...)
                    end
                    return fn
                end} )
...
function StateGraphInstance:Update()
    ...
    local should_speed_up = function(_self)
        -- return (_self.currentstate and ( _self.currentstate.name == "attack" or _self:HasStateTag("attack") ))
        return _self.inst ~= GetPlayer() and (_self.currentstate and ( _self.currentstate.name == "attack" or _self:HasStateTag("attack")))
    end
	
    local ENEMY_MULTI_ATTACK_SPEED = math.clamp(__xue__.ENEMY_MULTI_ATTACK_SPEED or 1, 1, 9999)
    local multi = ENEMY_MULTI_ATTACK_SPEED
    -- local fix = 0.499
    -- fix = __xue__.FIX or 0.5
    -- local multi = math.clamp( ENEMY_MULTI_ATTACK_SPEED, 0, self.inst.AnimState:GetCurrentAnimationLength()/FRAMES-1*FRAMES * fix )
    if self.currentstate.timeline and #self.currentstate.timeline > 0 then
        local last_time = self.currentstate.timeline[#self.currentstate.timeline].time
        multi = math.clamp( ENEMY_MULTI_ATTACK_SPEED, 0, math.max(1,last_time/FRAMES-2) )
    end
    

    ENEMY_MULTI_ATTACK_SPEED = multi
    if ENEMY_MULTI_ATTACK_SPEED > 1 and should_speed_up(self) then
        self.__mod_attack_time = self.__mod_attack_time or 0

        local deltaTime_total = ENEMY_MULTI_ATTACK_SPEED * dt
        local deltaTime 


        
        self.__mod_attack_time = self.__mod_attack_time + dt * ENEMY_MULTI_ATTACK_SPEED
        
        if self.inst.AnimState and not self.inst.__SetDeltaTimeMultiplier then
            self.inst.__SetDeltaTimeMultiplier_delay = 2
            -- self.inst.AnimState:SetDeltaTimeMultiplier(ENEMY_MULTI_ATTACK_SPEED)
            self.inst.__SetDeltaTimeMultiplier = true
        end

        self:UpdateState(deltaTime_total)
        
        -- local time = math.clamp(self.__mod_attack_time, 0, self.inst.AnimState:GetCurrentAnimationLength()-0.01)
        -- self.inst.AnimState:SetTime(time)
    else
        self.__mod_attack_time = 0
        if self.inst.AnimState and (self.inst.__SetDeltaTimeMultiplier or (self.inst.__SetDeltaTimeMultiplier_delay and self.inst.__SetDeltaTimeMultiplier_delay > 0)) then
            self.inst.__SetDeltaTimeMultiplier_later = 0
            self.inst.__SetDeltaTimeMultiplier = nil
            -- self.inst.AnimState:SetPercent("idle", 0)
            self.inst.AnimState:SetDeltaTimeMultiplier(1)
        end
        self:UpdateState(dt)
    end

    if self.inst.AnimState and self.inst.__SetDeltaTimeMultiplier_delay and self.inst.__SetDeltaTimeMultiplier_delay > 0 then
        self.inst.__SetDeltaTimeMultiplier_delay = self.inst.__SetDeltaTimeMultiplier_delay - 1
        if self.inst.__SetDeltaTimeMultiplier_delay <= 0 then
            self.inst.AnimState:SetDeltaTimeMultiplier(ENEMY_MULTI_ATTACK_SPEED)
        end
    end
