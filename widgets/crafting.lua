...
function Crafting:Close(fn)
	...
	crafttabs:HideSubTab(self.level)
end
...
function Crafting:Open(fn)
    ...
    self:Show() 
	-- Disable my highlight
	self.craftslots:Highlight("")
end

function Crafting:Refresh()
	self.craftslots:Refresh()
end

function Crafting:UpdateRecipes()
	TheSim:ProfilerPush("updaterecipes")
    self.craftslots:Clear()
    if self.owner and self.owner.components.builder then

        local recipes = GetAllRecipes()
		-- do some prework for the RecipeCategory support
		self.owner.components.builder:PrepareCategories()
        ...
        for k, v in pairs(recipes) do
            ...
            if show then
                ...
                if v.alt_ingredients or v.character_alt_ingredients then

                    local new_recipe = deepcopy(v)
                    
                    if v.alt_ingredients then
                        new_recipe.ingredients = deepcopy(new_recipe.alt_ingredients)
                    end

                    if v.character_alt_ingredients then
                        new_recipe.character_ingredients = deepcopy(new_recipe.character_alt_ingredients)
                    end

                    table.insert(self.valid_recipes, new_recipe)
                end
            end
        end
		...
		for i,v in pairs(self.craftslots.slots) do
			v:SetTooltip()
			v.crafttabs_overlay:Hide()
		end
        ...
        for k = 0, num do  
            ...
            if recipe then
                ...
                if show then
                    ...
                    if slot then
                        if recipe.alt_ingredients or recipe.character_alt_ingredients then
                            slot:ForceRecipe(recipe)
                        else
