...
local assets = 
{
    ...
    Asset("MINIMAP_IMAGE", "teleportato_hamlet_base"),    	
}
...
local function OnActivate(inst, doer)
	...
	if not inst.activatedonce then
		...
		inst.AnimState:PushAnimation("active_idle", true)
		inst.SoundEmitter:PlaySound("dontstarve_DLC003/common/crafted/teleportato/active", "teleportato_activate")
		inst.SoundEmitter:KillSound("teleportato_idle")
		inst.SoundEmitter:PlaySound("dontstarve_DLC003/common/crafted/teleportato/idle_LP", "teleportato_active_idle")
...
--		end)
		...
		if savemode == "porkland" or savemode == "survival" then
...
local function PowerUp(inst)
	...
	inst.SoundEmitter:PlaySound("dontstarve_DLC003/common/crafted/teleportato/power_on", "teleportato_on")
	inst.SoundEmitter:PlaySound("dontstarve_DLC003/common/crafted/teleportato/idle_LP", "teleportato_idle")
	
end

local partSymbols = {teleportato_hamlet_ring = "RING", teleportato_hamlet_crank = "CRANK", teleportato_hamlet_box = "BOX", teleportato_hamlet_potato = "POTATO" }
...
local function ItemGet(inst, giver, item)
	if inst.collectedParts[item.prefab] ~= nil then
		...
		inst.SoundEmitter:PlaySound("dontstarve_DLC003/common/crafted/teleportato/add_part")-- , "teleportato_addpart"
		TestForPowerUp(inst)
	end
end
...
local function MakeComplete(inst)
	print("Made Complete")
	inst.collectedParts = {teleportato_hamlet_ring = true, teleportato_hamlet_crank = true, teleportato_hamlet_box = true, teleportato_hamlet_potato = true }
end
...
local function fn(Sim)
    ...
    if SaveGameIndex:GetCurrentMode(Settings.save_slot) == "adventure" then
	    ...
	    anim:SetBuild("teleportato_hamlet")
	end
	...
	inst.collectedParts = {teleportato_hamlet_ring = false, teleportato_hamlet_crank = false, teleportato_hamlet_box = false, teleportato_hamlet_potato = false }
...
return Prefab( "common/objects/teleportato_hamlet_base", fn, assets, prefabs ) 
