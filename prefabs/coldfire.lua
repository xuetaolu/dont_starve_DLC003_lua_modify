...
local function fn(Sim)
    ...
    inst.components.fueled:SetUpdateFn( function()
        ...
        if GetSeasonManager() and GetSeasonManager():IsRaining() then
            rate = 1 + TUNING.COLDFIRE_RAIN_RATE*GetSeasonManager():GetPrecipitationRate()
        end
        if inst:GetIsFlooded() then 
            rate = rate + TUNING.COLDFIRE_FLOOD_RATE
        end 
        rate = rate +  GetSeasonManager():GetHurricaneWindSpeed() * TUNING.COLDFIRE_WIND_RATE
