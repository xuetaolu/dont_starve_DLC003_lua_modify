...
local function SelectLure(inst)    
    if inst.components.inventory then
        ...
        for k = 1, inst.components.inventory.maxslots do
            ...
            if item and item.components.edible and item.components.edible.foodstate ~= "COOKED" 
            and inst.components.eater:CanEat(item) and not item:HasTag("preparedfood") and not item.components.weapon 
            and not table.contains(lure_exclude, item.prefab) then
...
local function fn()
    ...
    inst.components.minionspawner.validtiletypes = validtiletypes
    inst.components.minionspawner:StartNextSpawn()
    ...
    -- Copied from this call, but minus the propagator.
