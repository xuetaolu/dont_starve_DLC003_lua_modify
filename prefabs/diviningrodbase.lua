...
local function OnUnlock(inst)
    ...
    if SaveGameIndex:IsModeShipwrecked() then
        inst.SoundEmitter:PlaySound("dontstarve_DLC002/common/teleportato_shipwrecked/teleportato_add_divining")
    else
...
local function OnReady(inst)
    if inst.components.lock:IsLocked() then
        ...
        if SaveGameIndex:IsModeShipwrecked() then
            inst.SoundEmitter:PlaySound("dontstarve_DLC002/common/teleportato_shipwrecked/teleportato_pulse", "pulse")
        else
