...
local function onupdate(inst)
	...
	local rain = Remap(math.max(sm:GetPrecipitationRate(), 0.5), 0.5, 1.0, 0.0, 1.0)
...
local function getstatus(inst, viewer)
	...
	if GetWorld() and GetWorld().Map:GetTileAtPoint(x, y, z) == GROUND.BEACH then
		return "SAND"
	else
		return "GENERIC"
	end
...
return Prefab( "common/objects/sand_castle", sandcastlefn, assets, prefabs),
		MakePlacer( "common/sandcastle_placer", "sand_castle", "sand_castle", anims[#anims] )
